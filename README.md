...
|`defaultother` (optional)|Text to pre-fill the "Other" text box on first load. Useful for preloading a previously collected "Other" response. Supports field references like `${field_name}`. Applied only on first load (when there is no saved plug-in metadata yet); on resume, the enumerator's saved text is used instead. Pair it with a dynamic default on the field that selects the "Other" choice to fully restore a prior "Other" answer.|
